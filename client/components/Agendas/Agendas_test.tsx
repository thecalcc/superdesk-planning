...
xdescribe('ManageAgendas', () => {
    describe('<ManageAgendasModal />', () => {
        ...
        describe('<EditCreate />', () => {
            ...
            it('can edit agenda', () => {
                function hasSaveButton(agendaInstance, options: {disabled: boolean}) {
                    return agendaInstance.findWhere((node) => {
                        const isDisabled =
                            node.props().hasOwnProperty('disabled') === options.disabled
                ...
                const editAgendaInstance = modal.find('EditAgenda').first();
                const nameField = editAgendaInstance.find('input').first();

                nameField.simulate('change', {target: {value: 'new name'}});

                expect(hasSaveButton(editAgendaInstance, {disabled: false})).toBe(true);

                // Name is mandatory
                nameField.simulate('change', {target: {value: ''}});

                expect(hasSaveButton(editAgendaInstance, {disabled: true})).toBe(true);
            });
